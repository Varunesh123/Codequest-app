...
import HackerEarthLogo from '../../assets/hackerearth-logo.png'; // Ensure path is correct

// Custom CSS for additional styling (same as above)
const styles = `
  .contest-card:hover {
...
const HackerEarthPage = () => {
  ...
  const fetchContests = async (showRefreshing = false) => {
    ...
    try {
      const res = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/contests/hackerearth?_=${Date.now()}`, {
        headers: {
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center space-x-3">
              <img src={HackerEarthLogo} alt="HackerEarth Logo" className="w-8 h-8" />
              <h2 className="text-xl font-bold">HackerEarth Contests</h2>
            </div>
          </div>
...
export default HackerEarthPage;