...
import CodeforcesLogo from '../../assets/codeforces-logo.png'; // Ensure path is correct
...
const CodeforcesPage = () => {
    ...
    const fetchContests = async (showRefreshing = false) => {
        ...
        try {
            const res = await fetch(
                `${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/contests/codeforces?_=${Date.now()}`,
                {
                    headers: {
    ...
    const getContestStatus = (contest) => {
        ...
        const end = contest.endTime ? new Date(contest.endTime) : new Date(start.getTime() + (contest.duration || 0) * 60 * 1000);
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-4">
                        ...
                        <div className="flex items-center space-x-3">
                            <img src={CodeforcesLogo} alt="Codeforces Logo" className="w-8 h-8" />
                            <h2 className="text-xl font-bold">Codeforces Contests</h2>
                        </div>
                    </div>
                                        ...
                                        <div className="flex flex-col items-end space-y-3 ml-5">
                                            ...
                                            {status.status === 'live' && (
                                                <a
                                                    ...
                                                    target="_blank"
                                                    rel="noreferrer"
                                                    className="flex items-center space-x-2 bg-red-500 hover:bg-red-600 text-white px-4 py-1 rounded-full text-sm font-semibold transition-colors"
                                                >
                                                    <FaPlay size={12} />
                                                    <span>JOIN</span>
                                                </a>
